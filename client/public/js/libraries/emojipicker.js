const EmojiPicker = function (options) {
    ...
    this.insertInto = this.options.trigger.map(item => item.insertInto);
    let emojiesHTML = '';
    let categoriesHTML = '';
    ...
    const functions = {
        ...
        // Chỉnh vị trí hiện thị tabl emoji
        rePositioning: (picker) => {
            ...
            if (window.innerHeight > pickerHeight) {
                picker.getBoundingClientRect().bottom > window.innerHeight ? picker.style.top = window.innerHeight - picker.offsetHeight - 120 + 'px' : false;
            }
        },
        ...
        render: (e, attr) => {
            ...
            // Fix lại cho không bị cố định
            this.insertInto = this.insertInto;
            ...
            const picker = `
                <div class="fg-emoji-container" style="left: ${position.left - 70}px; top: ${position.top - 430}px;">
